...
V7 核心改进（修复P0/P1所有问题）：
  ...
  [P0-1] 重构分类引擎 - 内容理解驱动，动态新建分类，不再依赖固定关键词
  [P0-2] 统一文档命名 - 格式：{分类}-{主题}，如"历史文化-钱谦益传"
  [P0-3] 同主题内容合并 - 跨分类检测同名文档，追加到主主题
  [P1-4] 图片/文档分类分离 - 图片按源文件夹，文档按AI分类
  [P1-5] 优化重复检测 - 主题词相似度 + hash 双重检测
  [P1-6] 修复统计报告 - 统计逻辑与实际处理保持一致
  [P1-7] 修复 IMA 同步 - 确保所有文档都同步

分类策略（V7.1 调整）：
  - 图片归档：按源文件夹分类（便于溯源查询）
  - 文档分类：按V7 AI分类（便于内容检索）

预设分类（可动态新建）：
  历史文化、营养健康、生活方式、教育育儿、旅游攻略、综合知识、其他

使用方法：
  python auto_process_all_v7.py           # 全自动处理所有图片
  python auto_process_all_v7.py --batch    # 分批模式（可中断续传）
  python auto_process_all_v7.py --init     # 仅初始化分批
  python auto_process_all_v7.py --progress # 查看进度
  python auto_process_all_v7.py --clear    # 清除分批状态
"""
...
class SmartContentAnalyzer:
    ...
    def extract_topic(self, text: str, category: str) -> str:
        ...
        # --- 策略1：人名提取 ---
        # 匹配2-4个汉字的人名（常见历史人物格式）
        person_names = []
        for line in lines[:15]:
            line = line.strip()
            # 匹配格式：XXX（年号）、XXX传、XXX简介 等
            name_match = re.search(
                r'([^\s，。、：！？]{2,4})[（(]?\s*(?:生平|传|简介|人物|大帝|皇帝|将军|先生|公|侯|王)', line
            )
            if name_match:
                person_names.append(name_match.group(1))
                break

            # 匹配 "XXX，字XXX" 或 "XXX（XXX-XXX）" 人物传记格式
            bio_match = re.match(
                r'^([^\s，。]{2,5})[，,]\s*(?:字|号|谥|生于|字号|原名)', line
            )
            if bio_match:
                person_names.append(bio_match.group(1))
                break
        ...
        for line in lines[:8]:
            ...
            if 4 <= len(line) <= 20 and re.match(r'^[\u4e00-\u9fa5]', line):
                # 排除无意义行
                if not any(skip in line for skip in ['http', 'www', '转载', '来源', '作者']):
                    return line[:15]
        ...
        if matched_kws:
            # 取前2个有意义的关键词
            meaningful = [kw for kw in matched_kws if len(kw) >= 2][:2]
            if meaningful:
                return ''.join(meaningful)

        # --- 策略4：兜底 ---
        timestamp = datetime.now().strftime('%m%d')
        return f"知识整理{timestamp}"
                ...
                if kw in line:
                    ...
                    if len(parts) > 1:
                        ...
                        if source:
                            ...
                            break
            for platform in ['小红书', '微信读书', '微信', '抖音', 'B站']:
                if platform in line:
...
def process_single_image(
    ...
    if not result.get('success'):
        print(f"  警告 OCR失败: {result.get('error')}")
        archiver.archive_image(image_path, "其他")
        return None

    text = result.get('text', '').strip()
    if not text or len(text) < 10:
        print("  警告 无文字内容或内容过少，跳过")
        archiver.archive_image(image_path, "其他")
        return None

    print(f"  OK 识别到 {len(text)} 个字符 (耗时 {ocr_time:.1f}s)")
    ...
    if source_info.get('platform'):
        print(f"  OK 来源: {source_info['platform']}")

    # 3. [P0-1][P0-2] AI分类 + 文档命名
    print("\n[3/6] AI分类与命名...")
    ...
    print(f"  OK 分类: {category} (置信度: {confidence:.0%} - {conf_label})")
    print(f"  OK 文档名: {doc_name}")
    if keywords:
        print(f"  OK 关键词: {', '.join(keywords[:5])}")
    if confidence < 0.15:
        logger.warning(f"[P0-1] 极低置信度 | {image_name} | {category} | {confidence:.0%}")

    # 4. [P1-5] 重复检测（hash + 主题相似度）
    print("\n[4/6] 重复检测...")
    if doc_manager.is_duplicate_hash(content_hash):
        print("  警告 内容hash重复，跳过存储")
    ...
    if is_new_doc:
        print(f"  OK 新建文档: {category}/{Path(md_file).name}")
    else:
        print(f"  OK 合并到已有文档: {Path(md_file).name}")
    ...
    if docx_file:
        print(f"  OK Word: {Path(docx_file).name}")
    else:
        print("  警告 Word生成失败（未安装python-docx）")

    # [P1-7] IMA同步
    print("\n[6/6] 同步到IMA...")
    if ima_syncer.rate_limited:
        print("  警告 IMA被限流，跳过")
    elif not ima_syncer.enabled:
        print("  - IMA未配置，跳过")
    else:
        ima_id = ima_syncer.sync_note(doc_name, clean_text, category, content_hash, md_file)
        if ima_id:
            print(f"  OK IMA同步成功")
        else:
            print("  警告 IMA同步失败")

    # [P1-4] 图片归档（按源文件夹分类，便于溯源）
    archived_path = archiver.archive_image(image_path, category)
    source_folder = archiver.get_source_folder(image_path)
    if archived_path:
        print(f"\n  OK 归档: 已处理图片/{source_folder}/{Path(archived_path).name}")
        print(f"       文档: 处理结果/{category}/{doc_name}.md")
...
def run_full_mode():
    """全自动处理模式（V7）"""
    print("=" * 60)
    print("全自动化图片处理工具 V7.0")
    print("OCR -> AI分类 -> 文档命名 -> 合并/新建 -> Word -> IMA -> 归档")
    print("=" * 60)
...
def print_usage():
    print("\n用法:")
    print("  python auto_process_all_v7.py           # 全自动处理所有图片")
    print("  python auto_process_all_v7.py --batch    # 分批模式（可中断续传）")
    print("  python auto_process_all_v7.py --init     # 仅初始化分批")
    print("  python auto_process_all_v7.py --progress # 查看进度")
    print("  python auto_process_all_v7.py --clear    # 清除分批状态")
