...
使用方法：
  python ima_sync.py          # 智能同步（变更的覆盖更新，未变更的跳过）
  python ima_sync.py --force  # 强制重新同步所有文件（删除后重新导入）
  python ima_sync.py --check  # 仅检查凭证是否有效
        ...
        if doc_name in sync_log and not force:
            ...
            if mtime <= last_sync_mtime:
                ...
                if doc_id:
                    ...
                    print(f"    [成功] 已覆盖更新（新doc_id: {doc_id[:12] if len(doc_id)>12 else doc_id}...）")
                    success_count += 1
                else:
                    fail_count += 1
        else:
