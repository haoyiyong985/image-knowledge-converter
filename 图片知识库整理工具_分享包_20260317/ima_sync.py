...
# -*- coding: utf-8 -*-
"""
ima_sync.py — 自动将处理结果同步到 ima 个人笔记
=================================================
功能：
  1. 读取 ima_config.txt 中的 API 凭证
  2. 扫描 处理结果/*.md 文件
  3. 通过 ima OpenAPI 自动导入或更新笔记
  4. 记录同步日志，避免重复导入

使用方法：
  python ima_sync.py          ← 同步所有 .md 文件（跳过已同步的）
  python ima_sync.py --force  ← 强制重新同步所有文件（覆盖更新）
  python ima_sync.py --check  ← 仅检查凭证是否有效，不执行同步
...
LOG_FILE    = BASE_DIR / "ima_sync_log.json"   # 记录已同步的文件和 doc_id
...
def check_credentials(client_id: str, api_key: str) -> bool:
    ...
    # ima 接口正常：HTTP 200 直接返回数据，无 retcode 包装
    # 只要没有 error 字段，且包含 is_end 或 note_book_folders 字段即为成功
    if "is_end" in result or "note_book_folders" in result or "docs" in result:
...
def import_md_to_ima(md_path: Path, client_id: str, api_key: str) -> str | None:
    ...
    # ima 返回格式：HTTP 200 直接返回数据，尝试多个可能的 doc_id 字段名
    doc_id = (
        result.get("doc_id")
    ...
    if doc_id:
        ...
        # 如果返回了 200 但没有 doc_id，打印完整响应以便调试
        print(f"    [警告] 请求成功但未找到 doc_id，完整响应：{result}")
        # 返回一个占位符，表示导入成功但无法追踪 doc_id
        return "imported_no_id"


def append_md_to_ima(doc_id: str, md_path: Path, client_id: str, api_key: str) -> bool:
    """
    向 ima 中已有笔记追加更新内容（用于文档有新增章节时）。
    成功返回 True，失败返回 False。
    """
    content = md_path.read_text(encoding="utf-8")

    # 追加时加一个时间戳分隔线，方便在 ima 里辨认更新位置
    ...
    # ima 返回 HTTP 200 且无 error 字段即为追加成功
    return True
...
def sync_all(force: bool = False):
    ...
    for md_path in md_files:
        ...
        # 获取文件最后修改时间（用于判断是否有更新）
        mtime = md_path.stat().st_mtime

        if doc_name in sync_log and not force:
            last_sync_mtime = sync_log[doc_name].get("mtime", 0)
            if mtime <= last_sync_mtime:
                print("    [跳过] 文件未变更，无需重新同步")
                skip_count += 1
                continue
            else:
                # 文件有更新，追加新内容
                print("    [更新] 文件有变更，追加更新到 ima...")
                doc_id = sync_log[doc_name].get("doc_id", "")
                if doc_id and doc_id != "unknown":
                    ok = append_md_to_ima(doc_id, md_path, client_id, api_key)
                    if ok:
                        sync_log[doc_name]["mtime"]      = mtime
                        sync_log[doc_name]["last_sync"]  = datetime.now().isoformat()
                        sync_log[doc_name]["sync_count"] = sync_log[doc_name].get("sync_count", 1) + 1
                        print(f"    [成功] 已追加更新（doc_id: {doc_id[:12]}...）")
                        success_count += 1
                    else:
                        fail_count += 1
                else:
                    # 没有有效 doc_id，重新导入
                    print("    [重新导入] 没有有效的 doc_id，作为新笔记导入...")
                    ...
                    if doc_id:
                        ...
                        print(f"    [成功] 已导入（doc_id: {doc_id[:12] if len(doc_id)>12 else doc_id}...）")
                        success_count += 1
                    else:
                        fail_count += 1
        else:
    ...
    if fail_count > 0:
        print("\n[提示] 有文件同步失败，可能原因：")
        print("  1. 网络不稳定 → 稍后重新运行脚本即可，已成功的不会重复导入")
