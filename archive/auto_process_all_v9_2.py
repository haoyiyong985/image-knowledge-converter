...
# -*- coding: utf-8 -*-
"""
全自动化图片处理工具 V9.2 - LLM全覆盖版（分次调用）
=====================================================
V9.2 核心升级：解决content字段JSON截断问题
  [V9.2-1] 分次调用 - 第1次LLM返回title/category/summary（短JSON不易坏）
  [V9.2-2] 第2次LLM返回整理后content（纯Markdown输出，无JSON包裹）
  [V9.2-3] 降级提取仍保留 - JSON仍失败时正则提取title/category/summary

V9.1 历史升级：
  [V9.1-1] LLM内容重写 - prompt新增content字段，一次调用完成命名+分类+摘要+内容整理
  [V9.1-2] 优先使用LLM内容 - final_content = llm_content（若LLM失败则退回OCR清理文本）
  [V9.1-3] 改进JSON解析 - 支持跨行content字段（DOTALL+代码块去除）
  [V9.1-4] LLM文本范围扩大 - 从500字扩至1200字，保留更多上下文
...
使用方法：
  python auto_process_all_v9_2.py           # 全自动处理
  python auto_process_all_v9_2.py --batch    # 强制分批模式
  python auto_process_all_v9_2.py --init     # 仅初始化分批
  python auto_process_all_v9_2.py --progress # 查看进度
  python auto_process_all_v9_2.py --clear    # 清除分批状态
"""
...
class LLMAnalyzer:
    ...
    def analyze(self, text: str, index: int) -> Dict:
        ...
        # 兜底：LLM全部失败
        if not result:
            logger.info(f"[LLM-{index}] 所有LLM不可用，使用正则兜底")
...
class ContentOrganizer:
    ...
    def _clean_text(self, text: str) -> str:
        ...
        for line in lines:
            ...
            # [V8.3] 过滤混合乱码行（小写字母夹中文，如"甘o亚a"）
            zh_chars = len(re.findall(r'[\u4e00-\u9fa5]', stripped))
            ltr_chars = len(re.findall(r'[a-z]', stripped))
            has_long_chinese = bool(re.search(r'[\u4e00-\u9fa5]{4,}', stripped))
            if ltr_chars >= 1 and zh_chars >= 1 and not has_long_chinese:
                continue
...
class SmartDocumentManager:
    ...
    def find_similar_doc(self, doc_name: str, content: str) -> Optional[Path]:
        ...
        for known_topic, known_path in self.known_topics:
            ...
            if known_path.exists():
                try:
                    existing = known_path.read_text(encoding='utf-8')
                    c1 = set(content[:400].replace(' ', '').replace('\n', ''))
                    c2 = set(existing[:400].replace(' ', '').replace('\n', ''))
                    if len(c1 & c2) < 15:
                        continue
                except Exception:
                    pass
    ...
    def _topic_similarity(self, t1: str, t2: str) -> float:
        """主题相似度：去后缀 + 核心实体 + 包含关系 + bigram Jaccard"""
        if not t1 or not t2:
            return 0.0

        def normalize(s):
            s = re.sub(r'(传|简介|人物|生平|整理|知识|合集|汇总|成就|贡献|概述|概览|简介)$', '', s).strip()
            s = re.sub(r'[（(]\s*[上下一二三四五六七八九十\d]\s*[)）]$', '', s).strip()
            s = re.sub(r'[上下一二三四]$', '', s).strip()
            for variant, unified in {
                '北宋': '宋朝', '南宋': '宋朝', '东汉': '汉朝', '西汉': '汉朝',
                '东晋': '晋朝', '西晋': '晋朝', '北魏': '魏朝', '东魏': '魏朝', '西魏': '魏朝',
            }.items():
                if variant in s:
        ...
        # 核心实体提取：从标题中提取2-3字的专有名词（人名/地名等）
        # 策略：标题开头的中文词就是核心实体，优先取2-3字（人名长度）
        def extract_core(topic, normed):
            # 优先取2字实体（最常见的人名长度）
            for length in (2, 3):
                m = re.match(r'^([\u4e00-\u9fff]{' + str(length) + r'})', normed)
                if m:
                    return m.group(1)
            # 退回原始标题
            m = re.match(r'^([\u4e00-\u9fff]{2,3})', topic)
            return m.group(1) if m else ''

        core1 = extract_core(t1, n1)
        core2 = extract_core(t2, n2)

        # 核心实体完全相同（如"钱谦益" == "钱谦益"）→ 高相似度
        if core1 and core2 and core1 == core2 and len(core1) >= 2:
            logger.info(f"[合并] 核心实体匹配: '{core1}' ∈ '{t1}' & '{t2}' → 0.80")
            return 0.80
...
def print_usage():
    print("\n用法:")
    print("  python auto_process_all_v9_2.py           # 全自动处理")
    print("  python auto_process_all_v9_2.py --batch    # 强制分批模式")
    print("  python auto_process_all_v9_2.py --init     # 仅初始化分批")
    print("  python auto_process_all_v9_2.py --progress # 查看进度")
    print("  python auto_process_all_v9_2.py --clear    # 清除分批状态")
