...
处理流程：
  ...
  4. 重复检测
  5. 文档生成（Markdown + Word）
  6. IMA同步
  7. 图片归档（按主题）

使用方法：
  python auto_process_all_v5.py
"""
...
class SmartDocumentGenerator:
    """智能文档生成器 - 支持主题分组、文档合并、Markdown+Word双格式"""
    ...
    def generate_word(self, text: str, theme: str, doc_name: str, image_name: str,
        ...
        if existing_md_file and Path(existing_md_file).exists():
            try:
                ...
                # 提取正文内容（去掉头部元信息）
                main_content = md_content.split('---')[2] if '---' in md_content else md_content
